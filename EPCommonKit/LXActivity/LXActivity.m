...
#define SHAREBUTTON_WIDTH                       65
#define SHAREBUTTON_HEIGHT                      65
#define SHAREBUTTON_INTERVAL_WIDTH              30
...
- (void)creatButtonsWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle shareButtonTitles:(NSArray *)shareButtonTitlesArray withShareButtonImagesName:(NSArray *)shareButtonImagesNameArray
{
        ...
        if (shareButtonImagesNameArray.count > 0) {
            ...
            for (int i = 1; i < shareButtonImagesNameArray.count+1; i++) {
                ...
                if (self.isHadTitle == YES) {
                    [shareButton setFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/3*(line-1) + [UIScreen mainScreen].bounds.size.width/6 - SHAREBUTTON_WIDTH/2, self.LXActivityHeight+((column-1)*(SHAREBUTTON_INTERVAL_HEIGHT+SHAREBUTTON_HEIGHT)), SHAREBUTTON_WIDTH, SHAREBUTTON_HEIGHT)];
                }
                else{
                    if (column == 1) {
                        [shareButton setFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/3*(line-1) + [UIScreen mainScreen].bounds.size.width/6 - SHAREBUTTON_WIDTH/2, 20+((column-1)*(SHAREBUTTON_INTERVAL_HEIGHT+SHAREBUTTON_HEIGHT)), SHAREBUTTON_WIDTH, SHAREBUTTON_HEIGHT)];
                    }else
                    {
                        [shareButton setFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/3*(line-1) + [UIScreen mainScreen].bounds.size.width/6 - SHAREBUTTON_WIDTH/2, SHAREBUTTON_INTERVAL_HEIGHT+((column-1)*(SHAREBUTTON_INTERVAL_HEIGHT+SHAREBUTTON_HEIGHT)), SHAREBUTTON_WIDTH, SHAREBUTTON_HEIGHT)];
                    }
                }
    ...
    if (shareButtonTitlesArray) {
        if (shareButtonTitlesArray.count > 0 && shareButtonImagesNameArray.count > 0) {
            for (int j = 1; j < shareButtonTitlesArray.count+1; j++) {
                ...
                //有Title的时候
                if (self.isHadTitle == YES) {
                    [shareLabel setFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/3*(line-1) + [UIScreen mainScreen].bounds.size.width/6 - SHARETITLE_WIDTH/2, self.LXActivityHeight+SHAREBUTTON_HEIGHT+((column-1)*(SHARETITLE_INTERVAL_HEIGHT)), SHARETITLE_WIDTH, SHARETITLE_HEIGHT)];
                }
    ...
    if (cancelButtonTitle) {
        ...
        if (self.isHadTitle == YES || self.isHadShareButton == YES) {
            [cancelButton setFrame:CGRectMake(cancelButton.frame.origin.x, self.LXActivityHeight + 10, cancelButton.frame.size.width, cancelButton.frame.size.height)];
        ...
        UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, CGRectGetMinY(cancelButton.frame) - 1, self.backGroundView.frame.size.width, 1)];
...
- (UIButton *)creatCancelButtonWith:(NSString *)cancelButtonTitle
{
    ...
    cancelButton.frame = CGRectMake(0, BUTTON_INTERVAL_HEIGHT, [UIScreen mainScreen].bounds.size.width, BUTTON_HEIGHT);
...
{
    UILabel *shareLabel = [[UILabel alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width/3*(line-1) + [UIScreen mainScreen].bounds.size.width/6 - SHARETITLE_WIDTH/2, SHARETITLE_INTERVAL_HEIGHT+((column-1)*(SHARETITLE_INTERVAL_HEIGHT)) + 5, SHARETITLE_WIDTH, SHARETITLE_HEIGHT)];
    if (column == 1) {
        shareLabel.frame = CGRectMake([UIScreen mainScreen].bounds.size.width/3*(line-1) + [UIScreen mainScreen].bounds.size.width/6 - SHARETITLE_WIDTH/2, SHARETITLE_INTERVAL_HEIGHT - 10 +((column-1)*(SHARETITLE_INTERVAL_HEIGHT)) + 5, SHARETITLE_WIDTH, SHARETITLE_HEIGHT);
    }
